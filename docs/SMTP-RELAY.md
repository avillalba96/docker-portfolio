...
```
Visitante → portfolio-front → portfolio-back → mail-service → Brevo SMTP → Outlook
Otros Ubuntu → curl / script → mail-service → Brevo SMTP → avillalba96@outlook.com
```
...
curl -X POST http://mail-service:3025/v1/send \
  ...
  -d '{"to":"avillalba96@outlook.com","subject":"Test","text":"Hola"}'
```
